...
def filter_users_by_min_ratings(ratings: pd.DataFrame, min_ratings: int = 10) -> pd.DataFrame:
    """
    Filtra gli utenti che hanno meno di min_ratings valutazioni
    
    Args:
        ratings: DataFrame con le valutazioni
        min_ratings: Numero minimo di valutazioni richieste
...
def process_dataset():
    ...
    try:
        ...
        # Filtra gli utenti con meno di 10 valutazioni
...
def get_movie_catalog_for_llm(limit: int = None) -> str:
    ...
    try:
        ...
        print(f"Errore durante il recupero del catalogo film: {e}")
        # Restituisci un catalogo vuoto in caso di errore
        return "[]"

if __name__ == "__main__":
    # Esegui il processamento quando il file viene eseguito direttamente
    process_dataset() 